...
- manual_testing_result_status --> [Works]
...
warning: unused variable: `note_clone`
  --> src/main.rs:45:9
   |
45 |     let note_clone = Arc::clone(&note_playing);
...
warning: function `subtract_noise` is never used
  --> src/main.rs:96:4
   |
96 | fn subtract_noise(frequency: f32, noise_profile: &Vec<f32>) -> f32 {
   |    ^^^^^^^^^^^^^^

warning: function `frequency_to_note` is never used
   --> src/main.rs:183:4
    |
183 | fn frequency_to_note(frequency: f32) -> String {
    |    ^^^^^^^^^^^^^^^^^
...
[] 🔍 Amplitude Analysis - Min: -0.16905, Max: 0.31167, Mean: 0.00022, Median: -0.00126
...
CLI Level updates nicely.

#### Thoughts

#### Instructions

##### Note on preferred length of response

short to medium length please, avoid long response.

##### Note on session

Please begin to wind up this session, unless there are further changes.

##### Code related

Future step, once current issues closed: (confirm first); then, Please add the following:

- function Re: summary of threads used around 10 seconds into program, using a separate file.

- `bitrate.rs` which should !print `config.sample_format`, `config.channels`, and `config.sample_rate`", and calculate and print `Bitrate = Sample Rate × Bit Depth × Channels`.

##### Additions

Per your instructions, added:
...
Question: Please explain `LAST_UPDATE` as it pertains to 

`last_update` in pub fn `print_live_amplitude()` of `live_output.rs`, on lines [9 to 17].
