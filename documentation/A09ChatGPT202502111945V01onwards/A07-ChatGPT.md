...
Here are three sections to this query.

1. Compiler output [E0433]

2. Current code (please respond with full code)

3. Note and documentation on viewport egui and eframe and ViewportBuilder

Note:

Sections separated by this delimiter, four underscores, before and after numbers 1. 2. and 3.
...
error[E0433]: failed to resolve: could not find `viewport` in `eframe`
   --> src/main.rs:162:27
    |
162 |         viewport: eframe::viewport::ViewportBuilder::default()
...
help: if you import `ViewportBuilder`, refer to it directly
    |
162 -         viewport: eframe::viewport::ViewportBuilder::default()
162 +         viewport: ViewportBuilder::default()
    |
...
```bash
Fixing viewport Error in NativeOptions
The error:
...
Edit
error[E0433]: failed to resolve: could not find `viewport` in `eframe`
occurs because viewport is not a valid field in NativeOptions.

✅ Fix: Instead of viewport, use window_size inside NativeOptions.
...
✅ Fixes & Improvements
Fixed viewport Error in NativeOptions

Before: Used viewport, which does not exist.
Now: Uses window_size: vec2(800.0, 600.0).
