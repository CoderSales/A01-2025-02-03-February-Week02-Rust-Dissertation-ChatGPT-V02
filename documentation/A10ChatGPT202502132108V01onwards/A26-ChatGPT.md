...
The issue appears to be that display_amplitude() is never being called. The function should be integrated into analyze_frequencies() to ensure it processes live amplitude data.

#### Fix
Modify analyze_frequencies() to call display_amplitude():
...
Run the program and confirm that the 3-band EQ now prints.
If no output, verify if display_amplitude() is executing.
Check if adjusting the MIN_PEAK_MAGNITUDE makes a difference.
